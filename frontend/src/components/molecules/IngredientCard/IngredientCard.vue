...
<template>
  <button
    class="flex flex-col items-center gap-1 p-2 border-2 transition-all cursor-pointer w-20"
    :class="selected
  ...
  >
    <IngredientIcon
      ...
      size="md"
    />
    <span class="font-pixel text-[7px] text-ramen-cream text-center leading-tight">
      {{ ingredient.name }}
    </span>
    <span class="text-[9px] text-ramen-cream/50 text-center">
      {{ ingredient.nameJp }}
    </span>
  </button>
</template>